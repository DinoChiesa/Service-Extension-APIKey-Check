...
/**
 * ServiceCalloutTools provides utility methods for handling HTTP header and body mutations in Envoy
 * service callouts. These methods are used to construct responses by adding, removing, and
 * modifying headers and bodies, as well as setting HTTP status codes.
 */
public class ServiceCalloutTools {
   ...
   *
   * <p>This method allows headers to be added, removed, and optionally appends header values. The
   * response can also be configured to clear the route cache.
   *
  ...
  public static void addHeaderMutations(
    ...
    if (addHeaders != null) {
      for (Map.Entry<String, String> entry : addHeaders) {
        HeaderValueOption.Builder headerValueOptionBuilder =
   ...
   *
   * <p>This method sets the body of the response, or clears the body if no content is provided.
  ...
  public static void addBodyMutations(
    ...
    if (body != null) {
      ...
      if (Boolean.TRUE.equals(clearBody)) {
        logger.log(
            Level.WARNING, "Body and clearBody are mutually exclusive. clearBody will be ignored.");
      }
    } else {
   ...
   *
   * <p>This method modifies an {@link ImmediateResponse.Builder} to include an HTTP status, header
   * modifications, and optionally, a response body. Headers can be added and removed.
   *
  ...
  public static void buildImmediateResponse(
    ...
    if (headersToAdd != null) {
      for (Map.Entry<String, String> entry : headersToAdd.entrySet()) {
        HeaderValueOption headerOption =
            HeaderValueOption.newBuilder()
                .setHeader(
                    HeaderValue.newBuilder()
                        .setKey(entry.getKey())
   ...
   * @param msg Custom message to log before denying the callout. Defaults to "Callout DENIED." if
   *     null.
